...
IIS_REQUEST = (
    ...
    "203.0.113.74 Mozilla/5.0+(Windows+NT+10.0) "
                ...
                host_match = logindex.match_rule(Path(case), {
                    "client_match": "any", "requests": [{"clauses": [
                        {"field": "host", "operator": "equals",
                         "values": ["www.sample.example.test"]},
                    ]}],
                })
...
class IisW3CAccessLogTests(unittest.TestCase):
    ...
    def test_fields_header_gives_columns_their_forensic_meaning(self):
        ...
        self.assertIsNotNone(parsed)
        self.assertEqual("203.0.113.74", parsed["ip"])
