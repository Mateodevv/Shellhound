...
- GitHub Actions must run only when the user explicitly requests them. Keep
  workflows manual-only (`workflow_dispatch`); do not add push, pull-request
  or scheduled triggers. A request to commit, push or merge is not a request
  to run Actions.
