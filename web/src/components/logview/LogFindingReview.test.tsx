...
import { ArtifactWindow } from '../review/ArtifactWindow'

vi.mock('../../api', async orig => ({ ...(await orig<typeof import('../../api')>()), api: vi.fn(), post: vi.fn() }))
vi.mock('../enrichment/ArtifactEnrichment', () => ({ ArtifactEnrichment: ({ ids }: { ids: number[] }) => <div>Enrichment IDs: {ids.join(',')}</div> }))
