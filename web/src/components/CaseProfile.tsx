...
const empty: CaseProfile = { organization_id: '', pseudonym: '', summary: '', sectors: [], countries: [], first_seen: '', last_seen: '', software: [], vulnerabilities: [], marking: 'TLP:AMBER+STRICT' }
...
export function CaseProfileForm({ slug, info, onClose }: { slug: string; info: CaseInfo; onClose: () => void }) {
  ...
  const [profile, setProfile] = useState<CaseProfile>({ ...empty, ...info.profile })
  ...
  const change = <K extends keyof CaseProfile>(key: K, value: CaseProfile[K]) => setProfile((p) => ({ ...p, [key]: value }))
  const create = useMutation({ mutationFn: () => post<Organization>('/api/organizations', {}), onSuccess: (org) => {
    change('organization_id', org.id)
  ...
  const save = useMutation({ mutationFn: () => patch(`/api/cases/${slug}`, {
    reference: reference.trim(), profile: { ...profile, sectors: sectors.split(',').map((v) => v.trim()).filter(Boolean), countries: countries.split(',').map((v) => v.trim()).filter(Boolean) },
  }), onSuccess: () => {
  ...
  return <form className="flex flex-col gap-4" onSubmit={(event) => { event.preventDefault(); if (!invalidDates) save.mutate() }}>
    <CtiField label={tr('cti.caseId')}><input value={reference} onChange={(e) => setReference(e.target.value)} placeholder="SYN-2026-001" className={ctiInput} /><span>{tr('cti.caseIdHint')}</span></CtiField>
    <CtiField label={tr('cti.pseudonym')}><select value={profile.organization_id} onChange={(e) => change('organization_id', e.target.value)} className={ctiInput}>
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      <CtiField label={tr('cti.sectors')}><input className={ctiInput} value={sectors} onChange={(e) => setSectors(e.target.value)} /></CtiField>
      <CtiField label={tr('cti.countries')}><input className={ctiInput} placeholder="DE, AT" value={countries} onChange={(e) => setCountries(e.target.value)} /></CtiField>
    ...
    <fieldset className="flex flex-col gap-2"><legend className="mb-2 text-[13px] font-semibold">{tr('cti.software')}</legend>
      {profile.software.map((item, index) => <div key={index} className="flex gap-2">
        <input aria-label={tr('cti.softwareName')} placeholder={tr('cti.softwareName')} className={ctiInput} value={item.name} onChange={(e) => change('software', profile.software.map((v, n) => n === index ? { ...v, name: e.target.value } : v))} required />
    ...
    <fieldset className="flex flex-col gap-2"><legend className="mb-2 text-[13px] font-semibold">{tr('cti.vulns')}</legend>
      {profile.vulnerabilities.map((item, index) => <div key={index} className="flex flex-col gap-2 rounded-lg border border-[var(--line)] p-3">
        <div className="flex gap-2"><input aria-label={tr('cti.vulnName')} placeholder={tr('cti.vulnName')} className={ctiInput} value={item.name} onChange={(e) => change('vulnerabilities', profile.vulnerabilities.map((v, n) => n === index ? { ...v, name: e.target.value } : v))} required />
