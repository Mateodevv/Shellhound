...
import { CaseProfileForm } from '../casework/CaseProfile'
...
const preview: OpenCtiPreview = { preview_id: 'preview-1', case_reference: 'SYN-2026-001', fingerprint: 'fp', objects: [{ id: 'file--a', type: 'file', name: 'shell.php' }], iocs: [{ id: 1, value: file.value, type: 'path', selected: true, object_ids: ['file--a'], indicator_supported: false, indicator_suggested: false, warnings: [] }, { id: 2, value: hash.value, type: 'hash', selected: true, object_ids: ['file--a'], indicator_supported: true, indicator_suggested: true, warnings: [] }], relationships: [{ id: 4, src_id: 2, dst_id: 1, kind: 'hash-of', note: '', selected: true }], samples: [{ id: 'sample-1', display_path: 'web/shell.php', sha256: hash.value, size: 20, selected: false, available: true, reason: '', file_id: 'file--a' }], warnings: [], errors: [] }
beforeEach(() => {
  vi.clearAllMocks()
...
describe('reviewed transfer', () => {
  ...
  it('shows stored provenance and stale/own-only status without implying a clean verdict', () => {
    renderWithProviders(<OpenCtiDetails lookup={{ ioc_id: 2, status: 'own', checked_at: '2026-09-01', stale: true, entities: [{ id: 'file', type: 'File', name: hash.value, url: 'javascript:alert(1)', labels: [], sources: [{ name: 'Shellhound SYN-2026-001' }], reports: [{ name: 'SYN-2026-001' }], malware: [], relationships: [] }] }} />)
    expect(screen.getByText('Own exports only')).toBeInTheDocument()
    expect(screen.getByText('Stale result')).toBeInTheDocument()
    expect(screen.getByText('Shellhound SYN-2026-001')).toBeInTheDocument()
