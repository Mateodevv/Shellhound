// Workspace threat intelligence, local GeoIP and detection configuration.
...
export function Settings({ initialTab = 'intel' }: { initialTab?: Tab }) {
  ...
  return (
    <div className="flex max-w-6xl flex-col gap-5">
      <Card surface="raised" className="px-4 py-3">
        <h1 className="text-lg font-bold">{tr('settings.title')}</h1>
        <p className="mt-1 text-[12.5px] text-[var(--muted)]">
          {tr('settings.sub')}{' '}
          <span className="mono break-all" title={data?.path}>{data?.path ? shortPath(data.path) : ''}</span>
        </p>
      </Card>
