...
import {
  api, downloadUrl, post, type Actor, type ActorsResponse, type TraceRow,
} from '../api'
import { formatCount, formatDay, formatLogTime, relativeTime } from '../format'
import {
  Button, Chip, Drawer, EmptyState, SearchInput, Tag,
} from '../components/ui'
...
function TraceDrawer({ slug, ips, onClose }: {
  slug: string; ips: string[] | null; onClose: () => void
}) {
  ...
  const colorByClient = useMemo(() => {
    ...
    ips?.forEach((ip, i) => map.set(ip, palette[i % palette.length]))
    return map
  }, [ips])
