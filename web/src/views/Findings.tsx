...
export function Findings({ slug }: { slug: string; gotoView: (v: ViewId) => void }) {
  ...
  return (
    <div className="flex h-[calc(100vh-40px)] flex-col gap-3">
      ...
      {/* Der Detail-Drawer liegt hinten, Viewer und Trace davor: was man AUS
          dem Detail heraus öffnet, muss davor stehen. */}
      <ArtifactDrawer
        slug={slug}
        artifact={selected}
        roots={roots}
...
/** Der Detail-Drawer EINES ARTEFAKTS: alles, was zur Entscheidung nötig ist,
 *  in einer Ansicht — die Entscheidung selbst steht oben, weil sie der Grund
 *  ist, aus dem man hier ist. */
function ArtifactDrawer({ slug, artifact, roots, collected, onClose, onTriage,
                          onView, onTrace }: {
  slug: string
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="rounded-xl border border-[var(--line)] bg-[var(--panel-2)] px-3 py-3">
          <div className="mb-2 text-[12.5px]">
            Ist dieses {KIND_LABEL[kind] ?? 'Artefakt'}{' '}
        ...
        {preview && !preview.error && !preview.binary && preview.lines && (
          <Block title={<>
            ...
            <pre className="mono max-h-72 overflow-auto rounded-lg bg-[var(--code-bg)] px-0 py-2 text-[11.5px] leading-relaxed text-[#e6edf3]">
        ...
        <Block
          ...
          {ips.length ? (
            <div className="flex flex-col gap-1">
              {ips.map((h) => (
                <div key={h.ip}
                  className="group flex items-center gap-2 rounded-lg bg-[var(--panel-2)] px-3 py-1.5 text-[12px]">
