...
# Apache:  [Fri Jun 26 05:19:59.123456 2026] [php:error] [pid 123] [client 192.0.2.4:5] PHP Fatal error: ...
# Nginx:   2026/06/30 11:52:26 [error] 1234#0: *5 FastCGI sent in stderr: "PHP message: ..."
_APACHE_RE = re.compile(
    r"^\[(?P<time>\w{3} \w{3} [ \d]?\d \d{2}:\d{2}:\d{2}(?:\.\d+)? \d{4})\]\s*"
    r"(?:\[(?P<module>[^\]]*)\]\s*)?"
    r"(?:\[pid[^\]]*\]\s*)?"
    # `[client 192.0.2.4:41234]` but also `[client [2001:db8::1]:41234]` --
...
# Apache writes `[client 192.0.2.4:41234]` for IPv4 and
