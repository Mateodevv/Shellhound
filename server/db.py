...
def _relativize_ioc_paths(conn):
    ...
    `sample-webroot/images/x.phtml` -- no longer absolute, so the loop
...
def relative_to_evidence(roots, path):
    ...
    `sample-webroot/images/sample-shell.phtml` -- where `sample-webroot` is
    ...
    `/images/sample-shell.phtml`, and so does every other view in this tool. Two
